...
S3_BUCKET_NAME = 'openskytrax'  # Nom du bucket S3
...
def send_to_kafka(producer, data, key_prefix):
    if data:
        try:
            ...
            producer.flush()
            print(f"Données envoyées avec succès au topic {KAFKA_TOPIC} avec le préfixe {key_prefix}.")
    ...
    try:
        ...
        while True:
            ...
            if msg.error():
                ...
                # Si les données sont de type Skytrax
                if "Airline" in message_data:
                    skytrax_data.append(message_data)
                # Si les données sont de type OpenSky
                elif "icao24" in message_data:
                    opensky_data.append(message_data)

                # Sauvegarder les données dans S3 lorsqu'on a collecté toutes les données
                if len(skytrax_data) > 0:
                    ...
                    skytrax_data = []  # Réinitialiser les données Skytrax après l'envoi

                if len(opensky_data) > 0:
                    ...
                    opensky_data = []  # Réinitialiser les données OpenSky après l'envoi
    except KafkaException as e:
